...
class DecTigerEnv(gym.Env):
    ...
    def __init__(self, proj_dim=64, seed=0):
        super().__init__()
    ...
    def step(self, action):
        ...
        if ja == ("listen", "listen"):
            ...
            if ja == ("open-left", "open-left") and self.state=='tiger-right':
                reward = 2
            if ja == ("open-right", "open-right") and self.state=='tiger-left':
                reward = 2
            else:
                reward = -2
...
if __name__ == "__main__":
    env = DecTigerEnv(proj_dim=64, activation='relu', seed=123)
    (obs0, obs1) = env.reset()
    for _ in range(3):
